...
pub fn init_lean() {
    START.call_once(|| unsafe {
        ...
        init_lean_thread();
        let init_res = ffi::initialize_crafter__web_CrafterWeb(1);
...
fn head_to_response(status: u16, headers: Vec<(String, String)>, body: Body) -> Response<BoxBody> {
    let builder = Response::builder().status(status);
    let builder = apply_headers(builder, headers);
    builder.body(boxed(body)).unwrap_or_else(|_| {
        Response::builder()
            .status(StatusCode::INTERNAL_SERVER_ERROR)
            .body(boxed(Body::from("invalid response")))
            .unwrap()
    })
}
...
fn stream_push_body(req_id: u64, chunk: &[u8], is_last: bool) -> u64 {
    unsafe {
        ...
        // Ownership of chunk_arr is transferred to Lean (stored in stream queue).
...
async fn handle(
    ...
    if let Ok(ws) = WebSocketUpgrade::from_request_parts(&mut parts, &state).await {
        let payload = match wire::encode_request(
            ...
            Err(err) => {
                ...
                return Response::builder()
                    .status(StatusCode::INTERNAL_SERVER_ERROR)
                    .body(boxed(Body::from("encode error")))
                    .unwrap();
            }
        };
        ...
        match wire::decode_response(&resp_bytes) {
            ...
            Err(err) => {
                ...
                return Response::builder()
                    .status(StatusCode::INTERNAL_SERVER_ERROR)
                    .body(boxed(Body::from("decode error")))
                    .unwrap();
            }
        }
    }
    ...
    let body_bytes = match to_bytes(body).await {
        ...
        Err(err) => {
            ...
            return Response::builder()
                .status(StatusCode::INTERNAL_SERVER_ERROR)
                .body(boxed(Body::from("body read error")))
                .unwrap();
        }
    };
    let payload = match wire::encode_request(
    ...
    let resp_bytes = handle_sync(state.app_id, &payload);
    match wire::decode_response(&resp_bytes) {
        Ok(wire_resp) => head_to_response(wire_resp.status, wire_resp.headers, Body::from(wire_resp.body)),
        Err(err) => {
            warn!(error = %err, "failed to decode wire response");
            Response::builder()
                .status(StatusCode::INTERNAL_SERVER_ERROR)
